import { Diagnostic, Position, Range, TextEdit } from 'vscode-languageserver/node';
import { TextDocument } from 'vscode-languageserver-textdocument';
import { computeOpenBlocksFromText, buildEndTagFor } from './astUtils';
...
export function buildCodeActions(params: {
    ...
    if (m) {
      ...
      if (local) {
        ...
        actions.push({
          ...
          kind: 'quickfix',
          diagnostics: [d],
          edit: { changes: { [uri]: [TextEdit.replace({ start: from, end: to }, sanitized)] } }
        });
      }
    }
  }
  ...
  for (const d of diagnostics) {
    if (d.message.includes("Consider '<#-")) {
      actions.push({
        ...
        kind: 'quickfix',
        diagnostics: [d],
        edit: { changes: { [uri]: [TextEdit.replace(d.range, '<#-')] } }
      });
    }
    if (d.message.includes("Consider '-#>")) {
      actions.push({
  ...
  const stack = computeOpenBlocksFromText(text, offset, parseContent);
  if (stack.length) {
    ...
    actions.push({
      ...
      kind: 'quickfix',
      edit: { changes: { [uri]: [TextEdit.insert(range.end, `\n${indent}${tags}`)] } }
    });
  }
  ...
  if (selectionText && selectionText.length > 0) {
    actions.push({
      ...
      edit: { changes: { [uri]: [TextEdit.replace(range, `<#- ${selectionText} -#>`)] } }
    });
    actions.push({
      title: 'Wrap with <# ... #>',
      kind: 'refactor.rewrite',
  ...
  for (const d of diagnostics) {
    ...
    if (m) {
      ...
      actions.push({
        ...
        kind: 'quickfix',
        edit: { changes: { [uri]: [TextEdit.insert(insertAt, scaffold)] } }
      });
      const curInsert = range.start;
      actions.push({
        title: `Insert block '${name}' here`,
        kind: 'quickfix',
        edit: { changes: { [uri]: [TextEdit.insert(curInsert, scaffold)] } }
      });
    }
  }
