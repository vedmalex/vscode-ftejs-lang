...
function extractInstructionCodeView(originalText, ast, options = {}) {
    ...
    if (!tokens.length)
        return { code: '' };
    const quote = getHostAdapter(options.hostLanguage === 'typescript' ? 'javascript' : (options.hostLanguage || 'javascript')).stringQuote;
    const out = [];
    for (const t of tokens) {
        const raw = (t.start || '') + (t.content || '') + (t.end || '');
        if (t.type === 'text') {
...
function ensureNewlineSuffix(token, nextToken) {
...
function ensureBlockSeparation(result, textBuffer, token) {
...
function ensureBlockEndSeparation(tokens, currentIndex) {
...
function countTrailingNewlines(result) {
    let count = 0;
...
function trimTrailingNewlines(result, toRemove) {
...
function setTrailingNewlines(result, desiredCount) {
...
function hasFollowingContent(tokens, fromIndex) {
...
function countBlankLinesBefore(tokens, index) {
    let count = 0;
...
function countBlankLinesAfter(tokens, index) {
    let count = 0;
    ...
    const flushTextChunk = () => {
        ...
        try {
            if (settings?.format?.textFormatter) {
                ...
                let protectedText = textBuffer.replace(/(#{[^}]*}|!{[^}]*})/g, (match, _g, offset) => {
                ...
                for (let li = 0; li < lines.length; li++) {
                    ...
                    line = line.replace(/⟨EXPR_PLACEHOLDER_(\d+)⟩/g, (_m, num) => {
